...
df11 <- filter(sources, title==11)$html[[1]] %>% read_html %>% html_nodes("table:nth-child(-n+3)") %>% html_table %>%
  ...
  mutate_at(.vars=vars(特技, 槍兵, 戟兵, 弩兵, 騎兵, 兵器, 水軍, 登場, 探索, 父親), .funs=function(x) na_if(x, "")) %>%
  mutate_at(.vars=vars(統率, 武力, 知力, 政治, 魅力, 生年, 没年, 登場), as.integer) %>%
  rename(name=名前) %>% mutate(title="11", order=row_number()) %>% dplyr::select(title, order, name, everything())
check_dup(df11)
...
parse_table12_by_page <- function(x, header){
  d_main <- x %>% html_nodes("table") %>% html_table %>% map(function(x) as.character(unlist(x))%>% matrix(nrow=1, byrow=T) %>% as.data.frame(stringsAsFactors=F) %>% set_names(header)) %>% bind_rows %>% filter(名前読み!="武将名")
...
df12 %<>% dplyr::select(-`合計`, -`格付け`, -`格付け2`) %>% mutate_at(.vars=vars(統率, 武力, 知力, 政治, 義理, 勇猛, 相性, 誕生, 登場, 没年, 寿命), as.integer) %>%
...
df13 <- filter(sources, title==13)$html[[1]] %>% read_html %>% html_nodes("table") %>% html_table(header=T) %>% map(function(x) mutate_all(x, as.character)) %>%
  ...
  mutate(相性=na_if(相性, "?"), 重臣特性=na_if(重臣特性, "-"), 理想威名=na_if(理想威名, "？")) %>%
  mutate_at(.vars=vars(相性, 生年, 登場, 没年, 統率, 武力, 知力, 政治), .funs=as.integer) %>%
  rename(name=名前) %>% mutate(title="13", order=row_number()) %>% dplyr::select(title, order, name, everything())
