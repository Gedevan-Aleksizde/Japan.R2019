...
wan <- xml2::as_xml_document(paste0("<html>", "&#x29166;", "</html>")) %>% html_text()
...
ruby_b <- function(text, ruby){
  glue("`\\textbf{\\aruby{[text]}{[ruby]}}`{=latex}`<ruby><strong>[text]</strong><rp>(</rp><rt>[ruby]</rt><rp>)</rp></ruby>`{=html}", .open = "[", .close = "]")
}
ruby <- function(text, ruby){
  glue("`\\aruby{[text]}{[ruby]}`{=latex}`<ruby>[text]<rp>(</rp><rt>[ruby]</rt><rp>)</rp></ruby>`{=html}", .open = "[", .close = "]")
}
```
